...
public class OptimalCostAlgorithm {
	...
	long [][][] pCost;
	BigInteger [][][] bpCost;
	ArrayList<Integer> [][][] locationAllObjects;// This consider all possible(for combination size) location of All objects 
	
	// total cost and final location each objects for each time slot "t".
	//CostElements [][] Cost;
	bCostElements [][] bCost;
	ArrayList<Integer> [][] finalLocationAllObjects;
	ArrayList<Integer> [][] tempLocationAllObjects;// This shows just only the best location of all objects that is calculated based on the variable above(locationAllObjects).
